...
if [ "$shutdown" = "yes" ]; then
  ...
  PROXY=$proxy_name docker-compose -p gho-local -f local/docker-compose.yml down $shutdown_options || true
  exit 0
fi
...
PROXY=$proxy_name docker-compose -p gho-local -f local/docker-compose.yml up -d
...
docker ps -a -fname=gho-local

# Install the site.
if [ "$install_site" = "yes" ]; then
  # Ensure the file directories are writable.
  echo "Ensure the file directories are writable."
  docker exec -it gho-local-site chmod -R 777 /srv/www/html/sites/default/files /srv/www/html/sites/default/private

  # Install the dev dependencies.
  echo "Install the dev dependencies."
  docker exec -it -w /srv/www gho-local-site composer install

  # Install the subtheme.
  #echo "Install the common design subtheme if not present already."
  #docker exec -it -w /srv/www gho-local-site composer run sub-theme

  # Install the site with the existing config.
  if [ "$use_existing_config" = "yes" ]; then
    echo "Install the site with the existing config."
    docker exec -it gho-local-site drush -y si --existing-config minimal install_configure_form.enable_update_status_emails=NULL
  else
    echo "Install the site from scratch."
    docker exec -it gho-local-site drush -y si minimal install_configure_form.enable_update_status_emails=NULL
  fi

  # Import the configuration.
  docker exec -it gho-local-site drush -y cim
...
# Install the dev dependencies and re-import the configuration.
  ...
  # Import the configuration.
  docker exec -it gho-local-site drush -y cr
